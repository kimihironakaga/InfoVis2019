function main()
{
    ...
    for ( var i = 0; i < 256; i++ )
    {
        var S = i / 255.0; // [0,1]
        var R = 1.0;
        var G = 0.5*(1+Math.cos((S * Math.PI)));
        var B = 0.5*(1+Math.cos((S * Math.PI)));
    ...
    for ( var i = 0; i < nfaces; i++ )
    {
        ...
        var max = Math.max(Math.max(S0,S1),S2);
        var min = Math.min(Math.min(S0,S1),S2);
        S0=Math.round(255*((S0-min)/(max-min)));
        S1=Math.round(255*((S1-min)/(max-min)));
        S2=Math.round(255*((S2-min)/(max-min)));
